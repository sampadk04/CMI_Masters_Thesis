...
from models.lm import LM, GriffinCustomConfig
...
# I/O
out_dir = 'out_tiny_shakespeare_griffin' # this is where the checkpoints will be saved
eval_interval = 100
log_interval = 20
eval_iters = 10
eval_only = False
...
wandb_run_name = 'tiny-shakespeare-griffin'
...
ext_storage_dir = '/storage/hdd1/model/smnist-chkpt'
...
separator_token = 2**n_bits + 1 # Assuming pixel intensities are in range 0-2^n_bits-1
...
mnist_data_dir = os.path.join('data', dataset)
...
train_dataset = MNIST(data_dir, train=True, transform=transform, download=set_download)
val_dataset = MNIST(data_dir, train=False, transform=transform, download=set_download)
...
if use_separator:
    meta = {
        'vocab_size': 2**n_bits + 2,  # Include separator token in vocab
        'itos': {i: i for i in range(2**n_bits + 2)},  # Identity mapping for pixel intensities and separator
        'stoi': {i: i for i in range(2**n_bits + 2)},
    }
else:
    meta = {
        'vocab_size': 2**n_bits + 1,  # Exclude separator token from vocab
        'itos': {i: i for i in range(2**n_bits + 1)},  # Identity mapping for pixel intensities and null token
...
#     # prepend the context with pad_token to make it block_size
#     if len(context) < block_size:
#         context = np.append(np.full(block_size - len(context), pad_token), context)

#     # extract the last block_size tokens from the context
...
print("Shape of context_xs:", context_xs.shape)
...
def get_batch(split, seed=0):
    ...
    if use_separator:
        jx = torch.randint(data.shape[1], (batch_size,))
    else:
        jx = torch.randint(data.shape[1] - 1, (batch_size,))
    
    X, Y = [], []

    for i, idx in enumerate(ix):
        xi = data[idx,:][0:jx[i]]
        yi = data[idx,:][0:jx[i]+1] # shift by 1, include 0th index incase of pad
        # check size of xi, if less than block_size, pad with pad_token, else select the last block_size tokens
        if len(xi) >= block_size:
            xi = xi[-block_size:]
            yi = yi[-block_size:]
        else:
            # pad
            xi = np.append(np.full(block_size - len(xi), pad_token), xi)
            yi = np.append(np.full(block_size - len(yi), pad_token), yi)
...
if model_type == 'griffin':
    model_config = GriffinCustomConfig(d_model=d_model, n_layers=n_layer, n_heads=n_heads, vocab_size=meta_vocab_size, local_attention_window=local_attention_window, dropout=dropout)
else:
...
def estimate_loss(seed):
    ...
    for split in ['train', 'val']:
        ...
        for k in range(eval_iters):
            ...
            with ctx:
                logits = model(X)
                loss = F.cross_entropy(logits.logits.view(-1, logits.logits.size(-1)), Y.view(-1), ignore_index=-1)
            losses[k] = loss.item()
...
while True:
    ...
    if iter_num % 25 == 0:
        # generate samples using context_x as input
        with torch.no_grad():
            with ctx:
                generated_tokens_batch = []
                # use current context
                current_contexts = context_xs.clone()
                for _ in range(max_new_tokens): # generate remaining tokens
                    logits = model(current_contexts)
                    logits = logits.logits[:, -1, :] / temperature
                    ...
                    # resample if the separator token is generated
                    for i in range(num_samples):
                        while (use_separator and next_tokens[i].item() == separator_token) or (next_tokens[i].item() == pad_token):
                            probs[i, next_tokens[i]] = 0 # set the probability of token to 0
                            next_tokens[i] = torch.multinomial(probs[i], num_samples=1)

                    # update generated tokens
                    generated_tokens_batch.append(next_tokens.squeeze().tolist())

                    # update current_context
                    current_contexts = torch.cat((current_contexts, next_tokens), dim=1)[:,-block_size:]
                
                # decode the generated tokens
                generated_images = []
                for i in range(num_samples):
                    generated_tokens = decode([token[i] for token in generated_tokens_batch])
                    generated_image = np.append(test_plots[i][:n_ctx_pixels], generated_tokens)
                    ...
                    if is_continous:
                        for j in range(1, image_size, 2):
                            generated_image[j] = np.flip(generated_image[j], axis=0)
